...
Las imágenes Node, Debian, Nginx y Redis se resuelven mediante tag legible más digest OCI multi-arquitectura. Selkies exige además una etiqueta de release versionada: sus etiquetas móviles `main` y `latest` no superan la puerta de contenedores aunque incluyan digest, porque el registro puede retirar el manifiesto cuando la etiqueta avanza. No reemplaces un digest manualmente: actualiza tag/digest juntos mediante un PR, reconstruye las cinco unidades, repite auditorías y ejecuta el smoke afectado. El pin inmoviliza la imagen base mientras el manifiesto permanezca publicado, pero los paquetes obtenidos por `apt` todavía requieren reconstrucciones periódicas y revisión de seguridad.
