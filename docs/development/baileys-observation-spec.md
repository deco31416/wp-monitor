...
| BO-CALL-28 | El observer se instala dormido antes de habilitar el backend, se arma por alcance firmado y generacion, impide contaminacion entre llamadas y expira por TTL; una perdida posterior a un inicio confirmado degrada la evidencia al motor libpcap. Arranque y salud periodica reconcilian contacto y `callId` con la captura autoritativa, recuperan solo coincidencias exactas y desarman alcances huerfanos o divergentes. |
...
| BO-CALL-30 | `stunTurnEvidence.version=1` correlaciona request/response por huella opaca y tupla de transporte, y conserva clase, metodo, rol ICE, `USE-CANDIDATE` y `CHANNEL-NUMBER`; ChannelData no retiene payload, solo se relaciona con un peer tras observar `CHANNEL-BIND` en la misma asignacion TURN y limita a 64 tanto los enlaces activos como la evidencia de canales. |
| BO-CALL-31 | Una confirmacion v4 por navegador exige par seleccionado y exitoso sin candidato relay, una IP ya elegible y coincidencia exacta de IP, puerto y protocolo con al menos 20 paquetes bidireccionales en negociacion o llamada activa. Al leer evidencia almacenada, esta coincidencia directa se exige solo a `direct_confirmed` o al componente directo confirmado de `mixed` sustentados por navegador; libros WebRTC/flujo contextuales no invalidan `relay_confirmed`, `direct_probable`, `mixed` probable ni `unresolved`. Los libros declarados deben seguir presentes y validos. |
...
| BO-CALL-33 | Historicos v2/v3 permanecen legibles; una conclusion v4 almacenada solo sobrevive si los libros declarados son validos y, cuando afirma confirmacion directa por navegador, esa coincidencia puede reconstruirse. |
...
| BO-CALL-35 | Al vencer su TTL, el observer toma un solo snapshot final, desarma la sonda y conserva el resultado por el mismo `callId` durante un maximo de una hora y 32 llamadas; la recuperacion es idempotente, no prolonga ni rearma la observacion y una ausencia real degrada explicitamente al motor de paquetes. |
| BO-CALL-36 | Si el capture-agent completa antes que el observer, el backend conserva la evidencia WebRTC exacta hasta recuperar el resultado de paquetes; reinicios, reconciliaciones y cierres concurrentes no pueden descartar ni cruzar evidencia entre `callId` o contactos. |
| BO-CALL-37 | Con el observer habilitado, inicio de paquetes y observacion forman una saga serializada: exito exige estados HMAC coincidentes y el inicio manual repite la comprobacion despues de dos segundos. Rechazo, timeout, inactividad o alcance divergente detienen compensatoriamente ambos componentes, limpian el estado no confirmado y producen un error controlado sin publicar `call-capture-started`. Una compensacion no verificable usa un error distinto y bloquea otro inicio sobre el alcance residual. |
| BO-CALL-38 | El observer conserva checkpoints sanitizados fuera del documento cada segundo, sin solapar lecturas; cada generacion sustituye su snapshot acumulativo y no suma consultas repetidas. Navegacion, destruccion del contexto principal o desconexion CDP invalidan la instrumentacion efectiva; el rearme solo ocurre en WhatsApp, bajo el mismo alcance y vencimiento original. El cierre nunca rearma. Maximos: 16 generaciones, 32 conexiones, 16 pares y 64 transiciones; truncacion e interrupciones son explicitas. Los checkpoints no sobreviven al reinicio del proceso ni recuperan intervalos perdidos. |
| BO-CALL-39 | `active` conserva la propiedad logica del alcance; `instrumentationActive` exige CDP conectado, sonda instalada/armada y verificacion reciente. El inicio coordinado rechaza instrumentacion explicitamente inactiva. Un stop cuyo desarme no puede verificarse falla controladamente, conserva el alcance en cierre para reintento y no extiende su TTL; el vencimiento conserva la limitacion de desarme no verificado si corresponde. |
