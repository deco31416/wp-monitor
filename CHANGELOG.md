...
- Preserved valid relay, probable and unresolved route assessments when reading stored browser/flow evidence: direct WebRTC pair matching now applies only to claims of independent direct confirmation. Added the OBS-29.9 empty-browser relay regression and JSON/HTML/PDF/ZIP parity checks without weakening missing-book or direct-confirmation validation.
- Preserved bounded WebRTC checkpoints outside the Chromium document across navigation, with scoped rearming under the original deadline, effective-instrumentation status, sanitized lifecycle diagnostics and explicit observation gaps. Repeated snapshots replace rather than duplicate evidence; unverified disarm retains scope ownership for retry. Checkpoints remain process-local and do not recover past lost observations.
...
- Replaced the retired Selkies browser-base manifest with the official versioned `v2.0.0rc0-debiantrixie` multi-architecture release digest and made the container gate reject moving `main`/`latest` Selkies tags, so identity and release retention no longer depend on a branch tag.
...
- INC-SELKIES-01 on 2026-09-14: the versioned `v2.0.0rc0-debiantrixie` OCI index resolved from GHCR and a clean `wa-browser` build passed. An isolated read-only smoke ran every process as UID 10001 with `cap_drop: ALL` and zero restarts, returned noVNC `200` and Selkies `401/200` without/with synthetic authentication, exposed CDP only inside the namespace, found the WhatsApp Web target and stopped without residue. The complete repository gate passed with 439 backend tests and 42 frontend tests.
